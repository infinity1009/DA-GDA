...
def get_eigenvector_centrality(nx_graph: nx.Graph, N: int):
    evc = nx.eigenvector_centrality_numpy(nx_graph, weight='edge_attr')
    evc = [evc[i] for i in range(N)]
    evc = torch.tensor(evc, dtype=torch.float32, device='cuda')
...
def get_degree(nx_graph: nx.Graph, N: int):
    deg = nx_graph.degree(weight='edge_attr')
    deg = [deg[i] for i in range(N)]

    deg = torch.tensor(deg, dtype=torch.float32, device='cuda')
...
def get_feature_weighted_mask(N, f, w, p: float, threshold: float = 0.5):
    w = w / w.mean() * p
    w = w.expand(N, f)
    w = w.where(w < threshold, torch.ones_like(w) * threshold)
...
def edge_dropping_8(edge_x):
    x_max = edge_x.max(dim=1).values
    weights = (x_max.max() - x_max) / (x_max.max() - x_max.mean())
    drop_mask = drop_edge_weighted_mask(weights, 0.1, 0.15)
...
def attr_perturbing_4(x, node_deg):
    ...
    return get_feature_weighted_mask(x.shape[0], x.shape[1], drop_feature_weights, 0.1 * attr_density, 0.15 * attr_density)
...
def attr_perturbing_5(x, node_deg):
    ...
    return get_feature_weighted_mask(x.shape[0], x.shape[1], drop_feature_weights, 0.15 * attr_density, 0.2 * attr_density)

# attribute perturbing--6


def attr_perturbing_6(x, node_evc):
    sign_x = torch.sign(x).float()
    attr_density = (sign_x.sum().sum() /
                    (sign_x.shape[0] * sign_x.shape[1])).item()
    drop_feature_weights = feature_weights(x, node_c=node_evc)
...
def attr_perturbing_7(x, node_evc):
    sign_x = torch.sign(x).float()
    attr_density = (sign_x.sum().sum() /
                    (sign_x.shape[0] * sign_x.shape[1])).item()
    drop_feature_weights = feature_weights(x, node_c=node_evc)
...
def attr_adding_3(x):
    ...
    N, f = x.shape[0], x.shape[1]
    x = x + torch.randn(N, f, device='cuda')
...
attr_plus = [None, attr_adding_1, attr_adding_2, attr_adding_3, attr_adding_4, attr_adding_5, attr_adding_6, attr_adding_7, attr_adding_8]
...
# not include edge_x (edge_dropping_8, edge_dropping_9)
'''
edge_minus = [None, edge_dropping_1, edge_dropping_2,
              edge_dropping_3, edge_dropping_4, edge_dropping_5, edge_dropping_6, edge_dropping_7, edge_dropping_8, edge_dropping_9, edge_dropping_10, edge_dropping_11]
'''
edge_minus = [None, edge_dropping_1, edge_dropping_2,
              edge_dropping_3, edge_dropping_4, edge_dropping_5, edge_dropping_6, edge_dropping_7, edge_dropping_10, edge_dropping_11]
