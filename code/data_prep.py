...
def prepare_data(args, l=20, e=1.0, f=1.0):
    ...
    data = Data(features, adj)  # PyG-form Graph, in CPU
    ...
    if e < 1.0:
        edge_x = None
        ebc = pkl.load(open(f'./metric/{args.dataset}_ebc_{e}.pkl', 'rb'))
        ebc = ebc.cuda()

        deg = pkl.load(open(f'./metric/{args.dataset}_deg_{e}.pkl', 'rb'))
        deg = deg.cuda()

        evc = pkl.load(open(f'./metric/{args.dataset}_evc_{e}.pkl', 'rb'))
        evc = evc.cuda()

    else:
        edge_x = pkl.load(open(f'./line_graph/embed_{args.dataset}.pkl', 'rb'))
        edge_x = edge_x.cuda()

        ebc = pkl.load(open(f'./metric/{args.dataset}_ebc.pkl', 'rb'))
        ebc = ebc.cuda()

        deg = pkl.load(open(f'./metric/{args.dataset}_deg.pkl', 'rb'))
        deg = deg.cuda()

        evc = pkl.load(open(f'./metric/{args.dataset}_evc.pkl', 'rb'))
        evc = evc.cuda()

    return graph, labels, index, nclass, nfeat, data, ebc, deg, evc, edge_x